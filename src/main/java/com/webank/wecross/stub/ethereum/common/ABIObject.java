...
public class ABIObject {
    ...
    public ABIObject newObject() {
        ...
        if (this.getAddressValue() != null) {
            abiObject.setAddressValue(new Address(this.getAddressValue().getValue()));
        }
        ...
        switch (type) {
            case VALUE: {
                switch (valueType) {
                    ...
                    case BYTES: {
                        Bytes32 tempB = TypeDecoder.decode(input, offset, Bytes32.class);
                        abiObject.setBytesValue(new Bytes(tempB.getValue().length, tempB.getValue()));
                        break;
                    }
                    case ADDRESS: {
